...
def test_fuzz_agent_skips_when_foundry_missing(tmp_path: Path) -> None:
    """FuzzAgent should record skipped status when Foundry is unavailable."""
    state_store = StateStore(tmp_path / "state.json")
    state_store.save(
        {
            "budget": {"spent": 0, "cap": 0},
    ...
    state = state_store.load()
    assert state["fuzz"]["status"] == "skipped"
    assert state["fuzz"]["reason"] == "foundry_unavailable"
