...
def test_llm_skipped_with_no_findings(tmp_path: Path) -> None:
    ...
    assert skipped["llm_synthesis"]["reason"] == "no_findings"
...
def test_llm_skipped_with_low_budget(tmp_path: Path) -> None:
    ...
    state = state_store.load()
    skipped = state["capabilities"]["skipped"]
    assert skipped["llm_synthesis"]["reason"] == "insufficient_budget"
    assert state["llm_synthesis"]["status"] == "skipped"
...
def test_llm_error_still_writes_report(tmp_path: Path) -> None:
    ...
    state = state_store.load()
    skipped = state["capabilities"]["skipped"]
    assert skipped["llm_synthesis"]["reason"] == "llm_error"
    assert state["llm_synthesis"]["status"] == "error"


def test_llm_skipped_when_unconfigured(tmp_path: Path) -> None:
    """LLM synthesis should skip when no endpoint is configured."""
    state = _base_state()
    state["findings"] = [
        {
            "category": "static",
    ...
    state = state_store.load()
    skipped = state["capabilities"]["skipped"]
    assert skipped["llm_synthesis"]["reason"] == "llm_unavailable"
