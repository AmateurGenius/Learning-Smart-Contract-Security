...
class StaticScan:
    ...
    def run(self, target_path: str) -> dict[str, Any]:
        ...
        signals, signal_evidence, slither_findings = self._extract_signals(slither_json)
        ...
        status = slither_json.get("status", "success")
        reason = slither_json.get("reason")
        skip_evidence = slither_json.get("evidence")
        ...
        if status == "success" and self._should_escalate(signals):
