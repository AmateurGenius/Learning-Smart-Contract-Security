...
def _check_capabilities(state: dict[str, Any], errors: list[str]) -> None:
    ...
    if "executed" not in capabilities or "skipped" not in capabilities:
        errors.append("Capabilities missing executed/skipped entries.")
        return
    for bucket in ("executed", "skipped"):
        entries = capabilities.get(bucket, {})
        if not isinstance(entries, dict):
            errors.append(f"Capabilities {bucket} is not a mapping.")
            continue
        for name, payload in entries.items():
            if not name:
                errors.append(f"Capabilities {bucket} entry missing name.")
                continue
            if not isinstance(payload, dict):
                errors.append(f"Capabilities {bucket} entry is not a mapping.")
                continue
            if bucket == "executed":
                for field in ("started_at", "finished_at", "status", "artifact_paths"):
                    if field not in payload:
                        errors.append(f"Capabilities executed entry missing {field}.")
            if bucket == "skipped":
                for field in ("reason", "evidence"):
                    if field not in payload:
                        errors.append(f"Capabilities skipped entry missing {field}.")
