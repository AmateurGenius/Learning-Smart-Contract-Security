...
def run_audit(target_path: str) -> Path:
    """Run the placeholder audit workflow and write a report."""
    state_store = StateStore(Path("state.json"))
    kernel = Kernel(state_store=state_store)
    return kernel.run(target_path)


def run_replay(run_path: str, rerun_tools: bool = False) -> Path:
...
def main() -> int:
    ...
    if args.command == "audit":
        run_audit(args.target_path)
        return 0
    if args.command == "replay":
        run_replay(args.run_path, rerun_tools=args.rerun_tools)
        return 0
